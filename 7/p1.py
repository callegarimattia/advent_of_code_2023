from termcolor import colored
from enum import Enum
from collections import Counter

file = open("day7/input.txt").readlines()

inputs = [
    (line.split(" ")[0].strip(), int(line.split(" ")[1].strip())) for line in file
]

strength = {
    "J": 0,
    "2": 1,
    "3": 2,
    "4": 3,
    "5": 4,
    "6": 5,
    "7": 6,
    "8": 7,
    "9": 8,
    "T": 9,
    "Q": 11,
    "K": 12,
    "A": 13,
}


class CardType(Enum):
    FIVE_OF_A_KIND = 1
    FOUR_OF_A_KIND = 2
    FULL_HOUSE = 3
    THREE_OF_A_KIND = 4
    TWO_PAIR = 5
    ONE_PAIR = 6
    HIGH_CARD = 7

    def __str__(self) -> str:
        return self.name


class Hand:
    hand_type: int = None
    bid: int = None
    card: str = None
    list_card: list = None

    def __init__(self, card, bid):
        self.card = card
        self.list_card = list(card)
        self.hand_type = self.get_type()
        self.bid = bid

    def get_type(self):
        counter = Counter(self.list_card)

        joker_count = counter["J"]
        if 0 < joker_count < 5:
            del counter["J"]
            counter[counter.most_common(1)[0][0]] += joker_count

        # Get the frequencies of the cards in descending order
        card_fequencies = [count for _, count in counter.most_common()]

        match card_fequencies:
            case [5]:
                return CardType.FIVE_OF_A_KIND
            case [4, 1]:
                return CardType.FOUR_OF_A_KIND
            case [3, 2]:
                return CardType.FULL_HOUSE
            case [3, 1, 1]:
                return CardType.THREE_OF_A_KIND
            case [2, 2, 1]:
                return CardType.TWO_PAIR
            case [2, 1, 1, 1]:
                return CardType.ONE_PAIR
            case _:
                return CardType.HIGH_CARD

    def __lt__(self, other: "Hand"):
        if self.hand_type != other.hand_type:
            return self.hand_type.value < other.hand_type.value

        for i in range(5):
            if self.list_card[i] != other.list_card[i]:
                return strength[self.list_card[i]] > strength[other.list_card[i]]


def print_results(hands):
    print(colored("RANK      HAND          BID", "green"))
    for i in range(len(hands)):
        print(colored(f"{i + 1}\t", "blue"), end="")
        print_hand(hands[i].list_card)
        print(colored(f"\t{hands[i].bid}", "magenta"))
    print(colored("RANK      HAND          BID", "green"))


def print_hand(hand):
    for card in hand:
        if card == "J":
            print(colored(card, "red"), end=" ")
        else:
            print(card, end=" ")


if __name__ == "__main__":
    hands = [Hand(x[0], x[1]) for x in inputs]
    # sort hands
    hands.sort(reverse=True)
    # print_results(hands)
    count = 0
    for i, hand in enumerate(hands):
        rank = i + 1
        count += hand.bid * rank
    print_results(hands)
    print(count)
